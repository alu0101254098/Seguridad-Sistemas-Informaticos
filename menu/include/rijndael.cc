...
string HexToBin1(string cadena) {
  string resultado;
...
Byte mult1(Byte b1, int pos, Byte Const) {
  for (int i = 0; i < pos; i++) {
    if (b1[7]){
      b1 <<= 1;
      b1 ^= Const;
...
Byte full_mult1(Byte b1, Byte b2, Byte Const){
  
  Byte result = Byte(string("00000000"));
  for (int i = 0; i < 8; i++) 
    if (b2[i]) 
      result ^= mult1(b1, i, Const);
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        b ^= full_mult1(e[k][i], emult[j][k], AES_byte);
      }
